...
const generateLinks = (links: Link[]) => {
  ...
  for (const link of links) {
    linksJsx.push(<a href={link.link} aria-label={link.label} className="Link" key={link.link} target="_blank" rel='noreferrer'><img id="header-image" src={"/website/assets/img/icons/" + link.image} alt={link.altText}/></a>)
  }
  return linksJsx;
};
